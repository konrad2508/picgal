...
call pyinstaller --onefile -i NONE --name picgal-server src/app.py
call rmdir /s /q build
call del picgal-server.spec 

call pyinstaller --onefile -i NONE --name picgal-sync-database src/sync_database.py
call rmdir /s /q build
call del picgal-sync-database.spec

call pyinstaller --onefile -i NONE --name picgal-rebuild-previews src/rebuild_previews.py
call rmdir /s /q build
call del picgal-rebuild-previews.spec

call pyinstaller --onefile -i NONE --name picgal-rebuild-samples src/rebuild_samples.py
call rmdir /s /q build
